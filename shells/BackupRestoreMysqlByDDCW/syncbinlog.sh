...
REMOTE_HOST=192.168.101.151
REMOTE_PORT=3366
REMOTE_USER=backup
REMOTE_PASSWORD=backup
...
#默认空的话, 就是当前目录
LOCAL_PRENAME=/data/backup/binlog/

mkdir -p ${LOCAL_PRENAME} >/dev/null 2>&1

#加个PID文件, 方便定时任务,判断, 比如: * * * * *  [[ -d /proc$(cat .mysqlBinlogSYNC.pid) ]] || sh mysqlBinlogSYNC.sh
PIDFILE=/run/mysqlBinlogSYNCbyDDCW_${REMOTE_HOST}_${REMOTE_PORT}.pid
[[ -f /proc/$(cat ${PIDFILE})/comm ]] && exit 0
echo $$ > ${PIDFILE}
...
function init_() {
	...
	#检查能否正常连接上去
	cat /dev/null > /tmp/._msyqlbinlogsynctmpbyddcw.tmp
	mysql -s -h ${REMOTE_HOST} -P ${REMOTE_PORT} -u ${REMOTE_USER} -p${REMOTE_PASSWORD} -e "show master status;"  > /tmp/._msyqlbinlogsynctmpbyddcw.tmp || exits "connect failed or exec 'show master status;' failed"
	#echo "/tmp/.${0}_msyqltmpbyddcw.tmp"
	export MASTER_CURRENT_BINLOG=$(awk '{print $1}'  /tmp/._msyqlbinlogsynctmpbyddcw.tmp)
	export MASTER_CURRENT_LSN=$(awk '{print $2}'  /tmp/._msyqlbinlogsynctmpbyddcw.tmp)
...
function main_() {
	...
	if [[ -z ${LOCAL_PRENAME} ]];then
		while :
		do
			mysqlbinlog  --skip-gtids --read-from-remote-server --raw --host=${REMOTE_HOST} --port=${REMOTE_PORT} --user=${REMOTE_USER} --password=${REMOTE_PASSWORD} --stop-never ${LOCAL_CURRENT_BINLOG} 2>/dev/null
