...
EOF
	...
	#system service
	cat << EOF > /usr/lib/systemd/system/zkPseudo.service
[Unit]
...
[Service]
Type=forking
ExecStart=for i in ${cluster_nodes};  do    /u01/zookeeper/bin/zkServer.sh start /u01/zookeeper/conf/zoo_\${i}.cfg; done
ExecStop=for i in ${cluster_nodes};  do    /u01/zookeeper/bin/zkServer.sh stop /u01/zookeeper/conf/zoo_\${i}.cfg; done
PrivateTmp=true
