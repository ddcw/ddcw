...
#返回一个cursor
def get_conn(MYSQL_HOST,MYSQL_PORT,MYSQL_USER,MYSQL_PASSWORD,DBNAME):
...
#插入数据
def insert_table(HOST, PORT, USER, PASSWORD, DBNAME, ACTION, TABLE_BASE_NAME, TABLE_ROWS, TABLE_PER_COMMIT, TABLE_THREAD, LOG, TABLE_COUNT, p, q):
	fake = Faker()
...
#随机压测
def bench(HOST, PORT, USER, PASSWORD, DBNAME, ACTION, TABLE_BASE_NAME, TABLE_ROWS, TABLE_PER_COMMIT, TABLE_THREAD, LOG, TABLE_COUNT, p, MODE, error_count):
	conn = get_conn(HOST, PORT, USER, PASSWORD, DBNAME)
	cursor = conn.cursor()
	fake = Faker()

	time.sleep(random.random()+random.random())#随机沉睡一段时间, 醒来就该干活了..
	def bench_write_sql():
		while True:
...
def _argparse():
	parser = argparse.ArgumentParser(description='Mysql 压测脚本, 其它类型的请看 https://github.com/ddcw/ddcw/tree/master/python')
	...
	parser.add_argument('--log', '-l' ,  action='store_true', dest='log',  help='准备数据的时候, 是否写日志(默认不写)')
	parser.add_argument('--report-interval', '-ri' ,  action='store', dest='report_interval', default=10, type=int, help='多少秒显示一次结果(默认10)')
	parser.add_argument('--thread', '-T' ,  action='store', dest='thread', default=8, type=int , help='并行度(默认8)')
...
def main(HOST, PORT, USER, PASSWORD, DBNAME, ACTION, TABLE_BASE_NAME, TABLE_ROWS, TABLE_PER_COMMIT, TABLE_THREAD, LOG, RUNTIME, REPORT_INTERVAL, TABLE_COUNT, MODE):
...
/*!*/;
		...
		for p in range(0,TABLE_THREAD):
			thread_list[p] = Process(target=insert_table, args=(HOST, PORT, USER, PASSWORD, DBNAME, ACTION, TABLE_BASE_NAME, TABLE_ROWS, TABLE_PER_COMMIT, TABLE_THREAD, LOG, TABLE_COUNT, p, q), daemon=True)
		for p in range(0,TABLE_THREAD):
			thread_list[p].start()
		for p in range(0,TABLE_THREAD):
			thread_list[p].join()
		...
		#放后台一直压测
		error_count = Value('d',0) #记录错误数的
		thread_list={}
		for p in range(0,TABLE_THREAD):
			thread_list[p] = Process(target=bench, args=(HOST, PORT, USER, PASSWORD, DBNAME, ACTION, TABLE_BASE_NAME, TABLE_ROWS, TABLE_PER_COMMIT, TABLE_THREAD, LOG, TABLE_COUNT, p, MODE, error_count), daemon=True)
		for p in range(0,TABLE_THREAD):
			thread_list[p].start()
		...
		for t in range(0, int(RUNTIME / REPORT_INTERVAL)):
			...
			current_errors = error_count.value
			time_internal = current_time - last_time
			print("[ {times} s ]  thds: {process}  tps: {tps}    qps: {qps}    err/s: {err}".format(tps=round((current_commit_rollback - last_commit_rollback)/time_internal,2), qps = round((current_query - last_query)/time_internal,2), process=TABLE_THREAD, times=(t+1)*REPORT_INTERVAL, err=round((current_errors-last_errors)/time_internal,2) ))
			time.sleep(REPORT_INTERVAL)
...
if __name__ == "__main__":
	...
	if parser.version :
		...
		main(parser.host, parser.port, parser.user, parser.password, parser.dbname, parser.action,  parser.table_name, parser.table_rows, parser.insert_per_commit, parser.thread, parser.log, parser.runtime, parser.report_interval, parser.table_count, parser.mode)
